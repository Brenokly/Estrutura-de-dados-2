...
public enum HashType {
   ...
   * 2 - Para casos em que as chaves são distribuídas de maneira aparentemente
   * aleatória.
   * 3 - Funciona bem quando o tamanho da tabela m é um número primo, o que ajuda
   * a minimizar colisões.
   * 
   * Cenário ideal:
  ...
  /*
   * No método da multiplicação, a chave é multiplicada por uma constante
   * fracionária A,
   * onde 0 < A < 1. A parte fracionária do resultado é multiplicada pelo tamanho
   * da tabela
   * e arredondada para baixo para gerar o índice. A função hash é dada por:
   * 
   * h(k) = ⌊m((k*A) mod 1)⌋
   * 
   * Onde k é a chave, A é a constante fracionária (geralmente relacionada à razão
   * áurea
   * ou outro número irracional), e m é o tamanho da tabela.
   * 
   * Quando usar:
   * 1 - Quando o tamanho da tabela hash não pode ser escolhido como um número
   * primo.
   * 2 - Quando é necessário evitar padrões regulares nas chaves, e a distribuição
   * precisa ser bastante uniforme.
   * 3 - Quando a eficiência e a distribuição uniforme das chaves são prioridades.
   * 
   * Cenário ideal:
   * Esse método é bom para situações onde o tamanho da tabela não pode ser
   * controlado
   * e você precisa de uma boa distribuição de chaves, sem precisar escolher um
   * número
   * primo como tamanho da tabela. É especialmente útil em sistemas com tabelas de
   * hash
   * de tamanho fixo.
   */
  ...
  /*
   * O método da dobra divide a chave em partes, e realiza operações específicas,
   * como
   * soma ou XOR, entre as partes da chave para gerar o índice da tabela hash.
   * Esse processo
   ...
   * 3. O tamanho da chave é reduzido em k dígitos, e uma operação (como XOR) é
   * aplicada
   * entre as partes.
   * 4. As dobras são feitas sucessivamente até obter o endereço-base de k
   * dígitos.
   * 
   * Exemplo:
   ...
   * 
   * Quando usar:
   * 1. Quando as chaves têm muitos dígitos e você deseja combinar diferentes
   * partes
   ...
   * 
   * Cenário ideal:
   * Esse método é ideal para sistemas com chaves longas e padronizadas,
   * especialmente quando
   * a chave pode ser dividida em partes e manipulada por operações bit a bit,
   * como XOR, para
   ...
   * padronizadas, como números de cartão, números de série ou matrículas. Pode
   * ser útil quando
  ...
  /*
   * O método de hash duplo é uma técnica que usa duas funções hash diferentes
   * para
   * calcular o índice da tabela hash. Se h1(k) é a primeira função hash e h2(k) é
   * a
   * segunda função hash, o índice é calculado como:
   * 
   * h(k, i) = (h1(k) + i * h2(k)) mod m
   * 
   * Onde k é a chave, i é o número de tentativas, m é o tamanho da tabela hash e
   * mod
   * é o operador módulo.
   * 
   * Quando usar:
   ...
   * 
   * 3 - Quando a função hash primária não é uniforme e a segunda função pode
   * ajudar
   * a distribuir as chaves de maneira mais uniforme.
   * 
   * Cenário ideal:
   * Esse método é útil quando a dispersão das chaves é crítica e a função hash
   * primária
   * não é suficiente para evitar colisões e você precisa testar diferentes
   * posições válidas.
   * Assim você garante que para cada chave tenha M posições para ser inserida.
   * Pode ser usado
   * em conjunto com outras técnicas de tratamento de colisões para garantir uma
   * boa distribuição das chaves.
   */

  // Essas informações acimas foram tiradas de pesquisas realizadas por mim, então
  // podem estar erradas!
